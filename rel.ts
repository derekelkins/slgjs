...
export default class Table<V extends Map<any>> {
    ...
    insert(row: V): void {
        ...
        for(let k in this.pathSpec) {
            ...
            if(subTable === undefined) index[val] = subTable = new Table([], this.pathSpec[k]);
            subTable.insert(row);
        }
    }
    ...
    private constructor(private readonly rows: Array<V>, private readonly pathSpec: PathSpec) { 
        this.indexes = {};
    }
